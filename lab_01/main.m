...
disp('Погрешность методом Cимпсона');
answer_for_method_simpson_f2_s = calculation_error(ideal_f2, answer_for_method_simpson_f2);
...
disp('Метод средних прямоугольников уравнение');
...
while m <= n+1
    if (m == 1) || (m == n+1)
        answer_for_method_trap = 0.5 .* F(x(m)) + answer_for_method_trap;
    else
        answer_for_method_trap = F(x(m)) + answer_for_method_trap;
    end
    m = m + 1;
end
answer_for_method_trap = answer_for_method_trap .* h;
disp('Метод трапеций уравнение');
